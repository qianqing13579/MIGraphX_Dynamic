...
argument hip_resize::compute(context& ctx,const shape& output_shape,const std::vector<argument>& args) const
{
    ...
    else
    {
        ...
        // 拷贝到gpu
        hipMemcpyAsync(shape_data.data(), shape_arg.data(), shape_arg.get_shape().bytes(), hipMemcpyDeviceToHost,ctx.get_stream().get());
